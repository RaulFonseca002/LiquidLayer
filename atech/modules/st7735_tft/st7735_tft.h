...
public:
    ...
    void begin();
    // Re-run the panel initialisation (double SWRESET + full init + rotation/BGR)
    // WITHOUT reallocating the driver or the canvas, then repaint the canvas.
    // Use after a warm reset or whenever the panel may have ingested garbage
    // while the MCU was booting (the panel keeps power across a chip reset).
    void reinit();
