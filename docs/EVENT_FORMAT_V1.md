...
Checkpoints are host-requested complete generic projections including pending commands, handle generations, and replay position. Retention may remove only data before a verified checkpoint by writing and validating a replacement generation, flushing it, and atomically replacing the old file. Arbitrary in-place deletion is forbidden.
